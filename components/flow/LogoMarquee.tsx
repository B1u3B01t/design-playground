...
export function LogoMarquee() {
  ...
  return (
    <div className="mt-10 max-w-md mx-auto w-full">
      <p className="text-sm font-medium text-foreground mb-1">Join 3,000+ designers</p>
      <p className="text-xs text-muted-foreground uppercase tracking-wider mt-4 mb-2">
      ...
      <div className="flex flex-wrap gap-3 items-center justify-center py-2">
        {labels.map((name) => (
          <span
            key={name}
            className="px-3 py-1.5 text-xs font-medium text-muted-foreground bg-foreground/5 rounded-lg"
          >
            {name}
          </span>
        ))}
