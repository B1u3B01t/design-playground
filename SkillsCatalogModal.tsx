...
import { Loader2, Plus, Search, Sparkles, Package, Link2, X, Trash2, ExternalLink, ArrowUpCircle } from 'lucide-react';
      ...
      >
        <div className="px-6 pt-6 pb-4 space-y-4">
          ...
          <div className="flex items-center gap-1 p-1 rounded-xl bg-stone-100 w-fit">
            <TabButton active={tab === 'browse'} onClick={() => setTab('browse')} icon={Sparkles}>
              Browse
            </TabButton>
  ...
  return (
    <>
      {[...groups.entries()].map(([category, skills]) => (
        <div key={category}>
          <SectionHeader icon={Sparkles} label={category} count={skills.length} />
...
function InstalledTab({
  ...
  return (
    <>
      ...
      {builtin.length > 0 && (
        <div>
          <SectionHeader icon={Sparkles} label="Built in" count={builtin.length} />
          <div className="space-y-2">
            {builtin.map((skill) => (
              <SkillRow
                key={skill.id}
